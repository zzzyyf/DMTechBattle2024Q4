...
### V2
- 既然发送是瓶颈，那就攒批
- batch size 固定，一批攒够所有线程的数据再发；server处理完一批请求再响应一批
- 这次是 sem_post 耗费了 90% 的时间，开 512 线程也只能跑到 50 秒左右，不如 V1

### V3
- 一个发送线程同步所有子线程压力太大，那就分多个发送线程
- 开 1024 线程，4连接可以跑到 19 秒左右，比 V1 略快

### V4
- 放在服务器上跑了一下，发现瓶颈出在 random 函数上
- 去除 random 函数之后，4096 线程可以跑进 5 秒内
- 用 Lehmer 算法替换了一下，性能基本没太多损耗，4096线程、16连接在 5 秒左右

### V5（TODO）
- 不应该攒够所有线程的数据再发，早发一批可以早点驱动相应的子线程继续
- 每个连接负责一组client，通过一个队列缓存待发送的请求
- 每次先发送队列中的请求，再接收响应，唤醒对应的client
